...
Node* RemoveMin(Node* node){
    if(!node->left) {
        ...
        std::cout << "RemoveMin->right  " << node->right << std::endl;
        return node->right;
    }
...
Node* cut(Node* node, T data, Node* par, bool is_right){

    if(!node) return 0;
    if(data < node->data)
        node->left = cut(node->left, data, node, false);
    else if(data > node->data)
        node->right = cut(node->right, data, node, true);
    else{
        how_much--;
        Node* left = node->left;
        Node* right = node->right;
        ...
        AVLTree* r_left =  new AVLTree(left);
        AVLTree* r_right =  new AVLTree(right);
        r_left->merge(r_right);
        if(is_right){
            par->right = r_left->root;
        }else{
            par->left =  r_left->root;
        }
